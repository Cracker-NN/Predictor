...
class Panel(object):
    ...
    def setupUi(self, MainWindow):
        ...
        MainWindow.setStyleSheet(f"color: rgb(255, 255, 255);\n"
                                 "                border-color: rgb(0, 170, 255);\n"
                                 "                font: 75 italic 14pt \"Consolas\";\n"
                                 "                alternate-background-color: rgb(26, 26, 26);\n"
                                 f"                background-color: rgb(45, 45, 68);")
        ...
        self.verticalScrollBar.setStyleSheet("QScrollBar:vertical{border: none;\n"
                                             "background: rgb(45, 45, 68);\n"
                                             "width: 14px;\n"
                                             "margin: 15px 0 15px 0;\n"
                                             "border-radius: 0px;}\n"
                                             "\n"
                                             "QScrollBar::handle:vertical {background-color: rgb(80, 80, 122);\n"
                                             "min-height: 30px;\n"
                                             "border-radius: 7px;}\n"
                                             "\n"
    ...
    def img_decoder(self, model_path: str, img_path: str, classes: any, resize: tuple[int, int] = (256, 256), channel: int = 3):
    ...
    def path(self):
        ...
        try:
            ...
            self.item.setText("\n".join(self.x))
            self.result = self.img_decoder(model_path=self.paths['Model'],
                                           img_path=self.img,
                                           classes=self.paths['Class'])
            self.prediction_result_window.setText(
                f"Predicted Result : {str(self.result)}")
            self.clock.setText(
                str(int(time.strftime('%S')) - int(self.t0)) + 's')
